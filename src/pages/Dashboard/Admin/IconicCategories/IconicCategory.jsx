...
const IconicCategory = () => {
    const { id } = useParams();
    const [iconicCategory, setIconicCategory] = useState({});
    const [allProducts, setAllProducts] = useState([]);

    // Fetch the special category
    useEffect(() => {
        fetch(`https://api.peakyonline.com/iconic_category/${id}`)
            .then(res => res.json())
            .then(data => {
    ...
    const selectedProducts = iconicCategory?.products || [];
    ...
    const handleAddProduct = (product) => {
        fetch(`https://api.peakyonline.com/iconic_category/${id}/add-product`, {
            method: "PATCH",
    ...
    const handleRemoveProduct = (product) => {
        fetch(`https://api.peakyonline.com/iconic_category/${id}/remove-product`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
        ...
        <div>
            ...
            <div className='bg-white p-5 my-4 rounded-xl shadow-xl flex justify-between items-center'>
                <h2 className='text-xl font-bold text-slate-600'>
                    {iconicCategory?.name} – Iconic Category
                </h2>
            </div>
